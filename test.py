...
# âœ… FIX 1: Fixed secret_key (os.urandom badata tha har restart pe session expire)
...
# âœ… FIX 2: Session lifetime globally set karo (pehle ye login route ke andar tha - galat jagah)
...
SOCKET_HUB_PORT = int(os.getenv("SOCKET_HUB_PORT", "5006"))  # âœ… FIXED: Changed from 7000 to 5006
SOCKET_HUB_READ_TIMEOUT = int(os.getenv("SOCKET_HUB_READ_TIMEOUT", "120"))  # âœ… FIXED: Changed from 5 to 120 seconds
...
def socket_hub_send_command(agent_id, command, data, timeout=None):
    ...
    print(f"   ðŸ“¤ [SOCKET_HUB] Sending to {agent_id}: command={command}, data keys={list((data or {}).keys())}")
    print(f"   ðŸ“¤ [SOCKET_HUB] Full payload: {cmd_payload}")

    try:
        _sock_send_json_line(sock_obj, wlock, cmd_payload)
        print(f"   âœ… [SOCKET_HUB] Command sent successfully to {agent_id}")
...
def _socket_hub_accept_loop():
    ...
    srv.listen(200)
    print("âœ… Socket hub listening on %s:%s" % (SOCKET_HUB_BIND, SOCKET_HUB_PORT))
    ...
    while True:
        try:
            ...
            print("âš  Socket hub accept error:", e)
            time.sleep(1)
...
def get_snapshot_dir(target_ip):
    ...
    if snapshot_dir:
        print(f"   ðŸ“ SNAPSHOT_DIR selected for {target_ip}: {snapshot_dir}")
    else:
        print(f"   âš ï¸  WARNING: No SNAPSHOT_DIR configured for {target_ip}")
...
def notify_admin_new_signup(user_type: str, full_name: str, address: str, email: str, phone: str,
    ...
    for admin_email in admins:
        # âœ… IMPORTANT: Let exception show in logs
...
def api_dealer_customers():
    ...
    try:
        print("\n" + "="*80)
        print("ðŸ“ž /api/dealer/customers called")
        ...
        # âœ… Function to get fresh access token
        def get_fresh_access_token():
            ...
            if not all([dealer_email, dealer_password, dealer_code]):
                print("âŒ Missing credentials in session")
                return None
            
            print(f"ðŸ”‘ Fetching fresh access token for {dealer_email}...")
            ...
            try:
                ...
                print(f"ðŸ”‘ Login response: {login_response.status_code}")
                ...
                if login_response.status_code == 200:
                    ...
                    if access_token:
                        print(f"âœ… Fresh token: {access_token[:50]}...")
                        session['external_access_token'] = access_token
                        session.modified = True
                        return access_token
                else:
                    print(f"âŒ Login failed: {login_response.text[:200]}")
                    
            except Exception as e:
                print(f"âŒ Token fetch error: {e}")
                traceback.print_exc()
            
            return None
        
        # âœ… Try with existing token first
        access_token = session.get('external_access_token')
        
        if not access_token:
            print("âš ï¸ No token in session, getting fresh token...")
        ...
        print(f"âœ… Using token: {access_token[:50]}...")
        
        # âœ… Fetch customers
        customers_url = "https://api.pgak.co.in/auth/dealer/customers"
        
        print(f"ðŸ“¡ Calling external API: {customers_url}")
        ...
        # âœ… If token expired, get fresh token and retry
        if response.status_code == 401:
            print("âš ï¸ Token expired, getting fresh token...")
        ...
        if response.status_code != 200:
            print(f"âŒ API error: {response.text[:500]}")
            return jsonify({
                "status": "error",
        ...
        data = response.json()
        print(f"âœ… Success! Customers: {data.get('count', 0)}")
...
def signup():
    ...
    import json  # âœ… added (safe local import)

    # âœ… Helper: external API response me se "message" nikalna
        ...
        with psycopg2.connect(**DB_CONFIG) as conn:
            with conn.cursor() as cur:
                ...
                if user_type == "admin":
                    ...
                    # âœ… SEND EMAIL TO NEW ADMIN USER
                    try:
                ...
                # ===== DEALER SIGNUP =====
                    ...
                    # âœ… NEW: Call external API to get dealer_code
                    ...
                    try:
                        ...
                        # âœ… IMPORTANT CHANGE: external error aaya to uska "message" nikaal ke wahi return karo
                        if api_response.status_code not in (200, 201):
                            ...
                            return jsonify({
                                "status": "error",
                                "message": api_msg  # âœ… yaha sirf msg
                            }), http_code
                    ...
                    # âœ… Save to database with dealer_code from external API
                    cur.execute("""
                        INSERT INTO user_signups
                    ...
                    # âœ… EMAIL ADMIN: New Dealer Request
                    try:
                ...
                # ===== DISTRIBUTOR SIGNUP (UNCHANGED) =====
                    ...
                    # âœ… EMAIL ADMIN: New Distributor Request
                    try:
                        notify_admin_new_signup(
...
#                         gst_no, company_name, pincode,
#                         distributor_code,
#                         dealer_code,  # âœ… Using dealer_code from external API
#                         user_type, "Pending"
#                     ))
...
def api_me_dealer_code():
    ...
    # âœ… Fast path: if already in session
...
def api_me_distributor_code():
    ...
    # âœ… Fast path: if you already stored it in session at login, return it directly
    ...
    try:
        ...
        # âœ… Cache in session so next calls donâ€™t hit DB
...
# ================================
# ðŸŽ¯ NEW ENDPOINT: SAVE QR SCANNED DATA TO DEVICE MASTER
# ================================
...
# # ================================
# # ðŸŽ¯ GET DEALER INFO BY ID (for QR scanner display)
# # ================================
...
#     except Exception as e:
#         print(f"âš  WS-Security header creation failed: {e}")
#         ws_security = create_ws_security_header(username, password)
...
#         else:
#             print("âŒ No RTSP URI found in response.")
#             return None

#     except Exception as e:
#         print(f"âŒ Exception fetching RTSP URI: {e}")
#         return None
...
#                 print(f"âœ“ Successfully scanned {device_url}")
                
#             except Exception as e:
#                 print(f"âœ— Error scanning {device_url}: {e}")
#                 continue
#         else:
#             print(f"âœ— Credentials failed for {device_url}")
...
def save_device_qr():
    ...
    try:
        ...
        if existing_device:
            ...
            # âœ“ DISTRIBUTOR ASSIGNMENT LOGIC
            if user_type == "distributor":
                # âŒ CRITICAL: Cannot reassign device to a different distributor
                ...
                # âŒ CRITICAL: Cannot assign to distributor if already has a dealer
            ...
            # âœ“ DEALER ASSIGNMENT LOGIC - CRITICAL RESTRICTIONS
            elif user_type == "dealer":
                # âŒ REJECT: Dealer trying to claim device already assigned to different dealer
                ...
                # âŒ REJECT: Cannot reassign device that already has customer
                ...
                # âŒ CRITICAL: Dealer WITHOUT parent distributor CANNOT claim device with any distributor
                if parent_distributor_id is None:
                    ...
                    # âœ“ Dealer HAS parent distributor - must match device's distributor
            ...
            # âœ“ CUSTOMER ASSIGNMENT LOGIC
            elif user_type == "customer":
                # âŒ REJECT: Cannot reassign to different customer
                ...
                # âœ“ VERIFY: If device has distributor, must match customer's parent distributor
                if assigned_distributor_id is not None and assigned_distributor_id != parent_distributor_id:
                    conn.rollback()
                    return jsonify({
                        "success": False,
                ...
                # âœ“ VERIFY: If device has dealer, must match customer's parent dealer
        ...
        if is_new_device:
            ...
            if user_type == "distributor":
                ...
                device_id = row[0]
                logger.info(
                    f"âœ… Device INSERTED by DISTRIBUTOR - ID: {device_id}, Serial: {serial_number}, Dist: {parent_distributor_id}"
                )

            elif user_type == "dealer":
                # âœ… NEW DEVICE: Dealer creates with OR WITHOUT parent distributor
                insert_sql = """
                    INSERT INTO public.device_master
                        (serial_number, dealer_id, distributor_id, customer_id, created_at, updated_at)
                    VALUES
                ...
                if parent_distributor_id:
                    logger.info(
                        f"âœ… Device INSERTED by DEALER - ID: {device_id}, Serial: {serial_number}, Dealer: {user_id}, Dist: {parent_distributor_id}"
                    )
                else:
                    logger.info(
                        f"âœ… Device INSERTED by DEALER (Independent) - ID: {device_id}, Serial: {serial_number}, Dealer: {user_id}, Dist: NULL"
                    )

            elif user_type == "customer":
                # âœ… NEW DEVICE: Customer creates with parent dealer and distributor
                insert_sql = """
                    INSERT INTO public.device_master
                        (serial_number, dealer_id, distributor_id, customer_id, created_at, updated_at)
                    VALUES
                ...
                device_id = row[0]
                logger.info(
                    f"âœ… Device INSERTED by CUSTOMER - ID: {device_id}, Serial: {serial_number}, Customer: {user_id}, Dealer: {parent_dealer_id}, Dist: {parent_distributor_id}"
                )

        else:
            # âœ… EXISTING DEVICE: UPDATE
            ...
            if user_type == "distributor":
                ...
                logger.info(
                    f"âœ… Device UPDATED by DISTRIBUTOR - ID: {device_id}, Serial: {serial_number}, Dist: {parent_distributor_id}"
                )

            elif user_type == "dealer":
                # âœ… UPDATE: Set dealer_id and distributor_id
                update_sql = """
                    UPDATE public.device_master
                ...
                logger.info(
                    f"âœ… Device UPDATED by DEALER - ID: {device_id}, Serial: {serial_number}, Dealer: {user_id}, Dist: {parent_distributor_id}"
                )

            elif user_type == "customer":
                # âœ… UPDATE: Set customer_id (final assignment)
                update_sql = """
                    UPDATE public.device_master
                ...
                logger.info(
                    f"âœ… Device UPDATED by CUSTOMER - ID: {device_id}, Serial: {serial_number}, Customer: {user_id}, Dealer: {parent_dealer_id}, Dist: {parent_distributor_id}"
                )
...
def save_device_qr_v2():
    ...
    try:
        ...
        logger.info(f"ðŸ” {user_type.upper()} {user_id} scanning: {serial_number}")

        parent_distributor_id = None
        parent_dealer_id = None

        # âœ… Distributor/Dealer validation
        if user_type == "distributor":
        ...
        if user_type == "customer":
            ...
            if is_new_device:
                ...
                return jsonify({
                    "success": False,
                    "message": "âŒ Device not issued by Admin. Contact your Dealer/Distributor first."
                }), 403
            ...
            if not assigned_dealer_id and not assigned_distributor_id:
                ...
                return jsonify({
                    "success": False,
                    "message": "âŒ Device not assigned to any Dealer/Distributor. Contact Admin."
                }), 403

            # ðŸš¨ Already assigned to ANY customer (same or different) â€” BLOCK
            if assigned_customer_id:
                if assigned_customer_id == customer_id:
                    conn.rollback()
                    return jsonify({
                        "success": False,
                        "message": "âŒ Device already registered to you."
                    }), 409
                else:
                    conn.rollback()
                    return jsonify({
                        "success": False,
                        "message": f"âŒ Device already assigned to another Customer."
                    }), 409

            # âœ… Safe to assign to this customer
            cur.execute(
        ...
        # =====================================================
        # ðŸš¨ DISTRIBUTOR FLOW
        # =====================================================
        elif user_type == "distributor":
            if is_new_device:
                # Fresh device â€” insert with distributor
                cur.execute("""
                    INSERT INTO public.device_master
                    (serial_number, dealer_id, distributor_id, customer_id, created_at, updated_at)
                    VALUES (%s, %s, %s, %s, %s, %s)
                ...
                # ðŸš¨ Already assigned to ANY distributor â€” BLOCK
                if assigned_distributor_id:
                    if assigned_distributor_id == parent_distributor_id:
                        conn.rollback()
                        return jsonify({
                            "success": False,
                            "message": "âŒ Device already issued to you."
                        }), 409
                    else:
                        conn.rollback()
                        return jsonify({
                            "success": False,
                            "message": f"âŒ Device already assigned to another Distributor."
                        }), 409

                # ðŸš¨ Already has a dealer assigned â€” BLOCK
                if assigned_dealer_id:
                    conn.rollback()
                    return jsonify({
                        "success": False,
                        "message": "âŒ Device already assigned to a Dealer."
                    }), 409

                # ðŸš¨ Already has a customer assigned â€” BLOCK
                if assigned_customer_id:
                    conn.rollback()
                    return jsonify({
                        "success": False,
                        "message": "âŒ Device already assigned to a Customer."
                    }), 409

                # âœ… Safe to assign distributor
                cur.execute(
        ...
        # =====================================================
            ...
            if is_new_device:
                # Fresh device â€” insert with dealer (+ distributor if linked)
                cur.execute("""
                    INSERT INTO public.device_master
                    (serial_number, dealer_id, distributor_id, customer_id, created_at, updated_at)
                    VALUES (%s, %s, %s, %s, %s, %s)
                ...
                # ðŸš¨ Already assigned to ANY dealer â€” BLOCK
                if assigned_dealer_id:
                    if assigned_dealer_id == parent_dealer_id:
                        ...
                        return jsonify({
                            "success": False,
                            "message": "âŒ Device already assigned to another Dealer."
                        }), 409
...
#         # =====================================================
#         # âœ… BEST + ROBUST DUPLICATE HANDLING (RACE-SAFE)
...
SMTP_CONFIG = {
    ...
    # âœ… This fixes hostname mismatch when SMTP_HOST is an alias
...
def _smtp_starttls_with_server_name(server: smtplib.SMTP, context: ssl.SSLContext, server_name: str):
    ...
    # âœ… Wrap with SNI + hostname verification against server_name
...
def send_email_otp(to_email: str, otp: str, otp_valid_minutes: int = 5) -> bool:
    try:
        ...
        # âš ï¸ Not recommended, only for testing
        ...
        if port == 465:
            ...
            with smtplib.SMTP(host, port, timeout=20) as server:
                ...
                if SMTP_CONFIG["use_tls"]:
                    # âœ… If tls_server_name is set, verify cert against that name
...
def admin_users():
    ...
    ðŸŽ¯ FIX: Excludes the currently logged-in admin from the list
    ðŸŽ¯ NEW: Also returns distributor's name/details for the distributor_code (if any)
    """
    try:
        if 'user_type' not in session or session['user_type'] != 'admin':
            return jsonify({"status": "error", "message": "Unauthorized"}), 401
        ...
        # âœ… LATERAL join ensures ONLY ONE distributor row is picked even if duplicates exist
        cursor.execute("""
            SELECT
                u.user_id,
                u.full_name,
        ...
        for user in users:
            users_dict[str(user['user_id'])] = {
                ...
                # âœ… NEW FIELDS (distributor details for that distributor_code)
...
def edit_user(user_id):
    ...
    âœ… FIX: status was required but was NOT being updated earlier
    """
    try:
        if 'user_type' not in session or session['user_type'] != 'admin':
            return jsonify({"status": "error", "message": "Unauthorized"}), 401
        ...
        """, (
            ...
            data['phone_number'],
            data['user_type'],
            data['status'],  # âœ… NEW: status update fix
            data.get('gst_no', ''),
            data['company_name'],
            data.get('pincode', ''),
...
def admin_pending():
    ...
    try:
        ...
        # ðŸŽ¯ Get the logged-in user's ID
        current_user_id = session.get('user_id')
...
# ================================
# ðŸŽ¯ NEW ENDPOINT FOR DISTRIBUTOR
# ================================
        ...
        try:
            with psycopg2.connect(**DB_CONFIG) as conn:
                with conn.cursor() as cur:
                    ...
                    # âœ… Session permanent rakho (24 hrs globally set hai upar)
                    session.permanent = True
                    
                    # âœ… NEW: For dealers, store credentials for re-authentication
                    external_token_success = False
                    if user_type.lower() == "dealer" and dealer_code:
                        # âœ… Store credentials in session for future token fetching
                        session["dealer_email"] = email
                        session["dealer_password"] = password  # Store original password
                        session["dealer_code"] = dealer_code
                        
                        try:
                            print(f"ðŸ” Getting initial access token for dealer: {email}")
                            ...
                            print(f"ðŸ“¡ External API Response Status: {ext_response.status_code}")
                            print(f"ðŸ“¡ External API Response Body: {ext_response.text[:500]}")
                            ...
                            if ext_response.status_code == 200:
                                ...
                                if access_token:
                                    # âœ… Store initial token in session
                                    session["external_access_token"] = access_token
                                    external_token_success = True
                                    
                                    print(f"âœ… External API authentication successful for dealer {email}")
                                    print(f"âœ… Initial token stored: {access_token[:50]}...")
                                else:
                                    print(f"âš ï¸ No access_token in response: {ext_data}")
                            else:
                                print(f"âŒ External API login failed: {ext_response.status_code}")
                                print(f"âŒ Response: {ext_response.text}")
                                
                        except requests.exceptions.Timeout:
                            print(f"â±ï¸ External API timeout for dealer {email}")
                        except requests.exceptions.ConnectionError as e:
                            print(f"ðŸ”Œ External API connection error: {e}")
                        except Exception as e:
                            print(f"âŒ External API error: {e}")
                            traceback.print_exc()
                    
                    # âœ… Mark session as modified
                    session.modified = True
                    ...
                    # âœ… Add warning if external token failed for dealer
                    if user_type.lower() == "dealer" and not external_token_success:
                        ...
                        print("âš ï¸ Warning: Dealer logged in but external API authentication failed")
                    
                    print(f"âœ… Login successful for {user_type}: {email}")
...
#             with psycopg2.connect(**DB_CONFIG) as conn:
#                 with conn.cursor() as cur:
#                     # âœ… CHANGE: also SELECT email so we can store it in session
#                     if "@" in contact:
#                         cur.execute("""
...
#                     # âœ… session saved fields used by /api/current-user
#                     session["user_type"] = user_type
#                     session["user_id"] = user_id
#                     session["full_name"] = full_name
#                     session["email"] = email  # âœ… NEW
...
def scan_device_with_ip():
    ...
    try:
        ...
        # âœ… FIX: SIRF YEH IP PE SCAN KARO, AUR KUCH NAHI
        print(f"ðŸ” Scanning ONLY this IP: {ip_address_value}")
        device_info = scan_device(ip_address_value)  # SIRF EK HI CALL

        if device_info and device_info.get("status") == "online":
            print(f"âœ… Device scan successful for IP: {ip_address_value}")
            return jsonify({
                "success": True,
            ...
            print(f"âŒ Device offline or not found: {ip_address_value}")
            return jsonify({
                "success": False,
        ...
        print(f"âŒ Error scanning device: {e}")
...
def update_device_status(info: dict, sys_status: str):
    ...
    if row is None:
        # First time this device is seen â€” INSERT
        if new_status == "ONLINE":
            cur.execute("""
                INSERT INTO device_status
                (ip_address, hostname, status, online_at, last_change_at)
        ...
        if last_status != new_status:
            ...
            # Same status but IP may have changed â€” update IP
            cur.execute("""
                UPDATE device_status
...
def device_discovery():
    """
    âœ… FIXED: Direct JOIN query for latest updated device
    ...
    # âœ… CRITICAL FIX: Single JOIN query to get latest updated device with IP
    TARGET_IP = None
    TARGET_SERIAL = None
    UPDATED_AT = None
    ...
    try:
        with psycopg2.connect(_build_dsn()) as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                # âœ… NEW APPROACH: One query with JOIN
                ...
                if result:
                    ...
                    # âœ… FIXED: Extract IP from ip_address field with deterministic priority.
                    # Old code used dict iteration order which is random â†’ caused wrong IP picked.
                    # New code: prefer wired (eth) â†’ wlan â†’ any other non-loopback interface.
                    # ip_data can be:
                    ...
                    print(f"\n{'='*80}")
                    print(f"ðŸŽ¯ LATEST UPDATED DEVICE:")
    ...
    print(f"\n{'='*80}")
    print(f"ðŸ“¡ HEARTBEAT RECEIVED")
    print(f"   Sender: {agent_ip}")
    print(f"   Target: {TARGET_IP} (last updated)")
    print(f"{'='*80}")

    # â›” IF NOT TARGET IP, REJECT
    if agent_ip != TARGET_IP:
        print(f"   âŒ REJECTED: {agent_ip} != {TARGET_IP}")
        print(f"{'='*80}\n")
        return jsonify({
            "ok": False,
    ...
    print(f"   âœ… ACCEPTED - Device discovery ENABLED")
    
    # âœ… FIXED: Use IP only as key (no port)
    key = agent_ip
    print(key,"---------------------------")
...
# DEBUG: LIST AGENTS
# ================================
# âœ… FIX: Removed duplicate AGENTS_LOCK/AGENTS={} that was here.
...
# checked AGENTS it was empty â†’ fell through to socket hub â†’ picked
...
def api_scan():
    ...
    âœ… FIXED Agent selection priority:
      1. agent_id/agent_ip provided in request body â†’ use that directly
      2. AGENTS dict (device_discovery verified target, PRIMARY) â†’ use TARGET IP
      3. Socket hub _SOCKET_AGENT_CONNS (SECONDARY, fallback only)
      4. No agent found â†’ 503

    âš ï¸  ROOT CAUSE OF BUG (now fixed):
         ...
         is stored there). If that agent is also in socket hub â†’ use it.
    ...
    # â”€â”€ Step 1: Get agent_id from request (optional) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Step 2: Auto-select agent if not provided â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # if not agent_id:
    #     print(f"   No agent_id in request â€” auto-selecting...")

    #     # âœ… FIX: PRIMARY = AGENTS dict (device_discovery verified target IP)
    ...
    #             agent_id = candidate
    #             print(f"   âœ… Auto-selected VERIFIED target from AGENTS dict: {agent_id}")
    #         else:
    #             # Target device is registered but socket not connected yet â€”
    #             # warn clearly instead of silently using a wrong device
    #             print(f"   âš ï¸  Verified target {candidate} not in socket hub!")
    #             print(f"   âš ï¸  Socket hub has: {socket_agents}")
    #             # Try other verified agents that ARE in socket hub
    #             for k in agents_keys[1:]:
    #                 if k in socket_agents:
    #                     agent_id = k
    #                     print(f"   âœ… Using next verified agent in socket hub: {agent_id}")
    #                     break
    #             if not agent_id:
    #                 print(f"{'='*70}\n")
    #                 return jsonify({
    ...
    #                 }), 503
    #     elif socket_agents:
    #         # âœ… SECONDARY fallback: no device_discovery verified agent yet,
    #         # use whatever is in socket hub (same as old behaviour, but only as fallback)
    #         agent_id = socket_agents[0]
    #         print(f"   âš ï¸  No verified agent â€” falling back to socket hub: {agent_id}")
    #         print(f"   âš ï¸  WARNING: This may not be the correct target device!")
    #         print(f"   âš ï¸  Ensure /device_discovery heartbeat is running on the target device.")
    #     else:
    #         print(f"   âŒ NO AGENTS FOUND ANYWHERE!")
    ...
    # â”€â”€ Step 2: Auto-select VERIFIED agent from AGENTS dict â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if not agent_id:
        print("   No agent_id in request â€” selecting verified target from AGENTS...")

        with AGENTS_LOCK:
            if AGENTS:
                # âœ… Pick most recently seen verified agent
                ...
                print(f"   âœ… Using verified target IP from AGENTS: {agent_id}")
            else:
                print("   âš ï¸  No verified agents in AGENTS dict â€” falling back to socket hub")
        ...
        if not agent_id:
            ...
            if socket_agents:
                agent_id = socket_agents[0]
                print(f"   âš ï¸  Fallback: using socket hub agent: {agent_id}")
            else:
                print(f"   âŒ No agents anywhere!")
                print(f"{'='*70}\n")
                return jsonify({
                    "status": "error",
    ...
    # â”€â”€ Step 3: Validate agent_id is in socket hub â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if not is_in_socket_hub:
        print(f"   âš ï¸  Requested agent '{agent_id}' NOT in socket hub")
    ...
    print(f"   âœ… Final agent_id: {agent_id}")

    # â”€â”€ Step 4: Send scan command via socket hub â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    scan_data = {"username": username, "password": password, "customer_id": customer_id}
    print(f"\nðŸ”Œ Sending scan command via socket hub to agent: {agent_id}")
    print(f"   ðŸ“¤ Data being sent to Pi: {scan_data}")
    print(f"   ðŸ“¤ customer_id value: {customer_id} (type: {type(customer_id).__name__})")
    ...
    print(f"   ðŸ“¥ Socket response: {sock_resp}")

    # â”€â”€ Handle async scan acknowledgment (scan_started: True) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if sock_resp and sock_resp.get("status") == "ok":
        ...
        if isinstance(payload, dict) and payload.get("scan_started") is True:
            print(f"   â³ Scan started asynchronously â€” polling for results...")
            ...
            if scan_results:
                print(f"   âœ… SCAN SUCCESS! Found {scan_results.get('count', 0)} devices")
                print(f"{'='*70}\n")
                return jsonify({
                    "status": "ok",
                ...
                print(f"   â±ï¸ Scan results timeout")
                print(f"{'='*70}\n")
                return jsonify({
                    "status": "error",
                    "message": "Scan timeout â€” agent is still scanning or no cameras found.",
        ...
        if isinstance(payload, dict) and "devices" in payload:
            print(f"   âœ… SCAN SUCCESS (sync)! Found {payload.get('count', 0)} devices")
            print(f"{'='*70}\n")
            return jsonify({
                "status": "ok",
        ...
        if "count" in sock_resp and "devices" in sock_resp:
            print(f"   âœ… SCAN SUCCESS (sync)! Found {sock_resp.get('count', 0)} devices")
            print(f"{'='*70}\n")
            return jsonify({
                "status": "ok",
    ...
    # â”€â”€ Handle error response from agent â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if sock_resp and sock_resp.get("status") == "error":
        error_msg = sock_resp.get("message", "Unknown error")
        print(f"   âŒ AGENT RETURNED ERROR: {error_msg}")
        print(f"{'='*70}\n")

        if "timeout" in error_msg.lower():
            return jsonify({
                "status": "error",
                "message": "Scan timeout â€” agent is busy or scanning a large network.",
                "details": error_msg,
    ...
    # â”€â”€ No response at all â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    print(f"   âŒ No response from agent (timeout or disconnected)")
    print(f"{'='*70}\n")
    return jsonify({
        "status": "error",
...
# âœ… NEW: Global dict to store async scan results
...
def _wait_for_scan_results(agent_id: str, username: str, password: str, max_wait_seconds: int = 40, poll_interval: int = 2) -> dict:
    ...
    print(f"   ðŸ” Polling for scan results with key: {scan_key}")
    ...
    while (time.time() - start_time) < max_wait_seconds:
        ...
        with SCAN_RESULTS_LOCK:
            if scan_key in SCAN_RESULTS:
                ...
                print(f"   âœ… Scan results received after {elapsed:.1f}s (attempt #{attempts})")
                return results
        
        # Log progress every 10 seconds
        if attempts % 5 == 0:  # Every 5 attempts (10 seconds if poll_interval=2)
            elapsed = time.time() - start_time
            print(f"   â³ Still waiting for scan results... {elapsed:.0f}s elapsed")
        
        time.sleep(poll_interval)
    
    # Timeout
    elapsed = time.time() - start_time
    print(f"   â±ï¸ Scan results polling timeout after {elapsed:.1f}s ({attempts} attempts)")
    return None
...
def api_scan_results():
    """
    âœ… NEW ENDPOINT: Agent POSTs scan results here after completing async scan.
    ...
    print(f"\n{'='*70}")
    print(f"ðŸ“¥ SCAN RESULTS RECEIVED FROM PI")
    print(f"   Agent ID: {agent_id}")
    print(f"   Username: {username}")
    print(f"   Password Hash: {password_hash}")
    print(f"   Count: {count}")
    print(f"   ðŸ“Œ user_id (top-level from Pi): {user_id_from_pi}")
    print(f"   ðŸ“Œ user_id type: {type(user_id_from_pi).__name__}")
    if devices:
        for i, dev in enumerate(devices[:3]):
            print(f"   ðŸ“Œ Device[{i}] user_id: {dev.get('user_id', 'NOT SET')} | ip: {dev.get('ip', dev.get('ip_address', 'N/A'))}")
        if len(devices) > 3:
            print(f"   ... and {len(devices) - 3} more devices")
    print(f"   ðŸ“Œ Full payload keys: {list(data.keys())}")
    print(f"{'='*70}")
    
    if not agent_id or not username or not password_hash:
        print(f"   âŒ Missing required fields")
        print(f"{'='*70}\n")
        return jsonify({
            "status": "error",
    ...
    print(f"   âœ… Scan results stored with key: {scan_key}")
    print(f"{'='*70}\n")
...
# âœ… NEW: Cleanup old scan results (prevent memory leak)
    ...
    while True:
        try:
            ...
            with SCAN_RESULTS_LOCK:
                ...
                if expired_keys:
                    print(f"ðŸ§¹ Cleaned up {len(expired_keys)} expired scan results")
        
        except Exception as e:
            print(f"âš ï¸ Error in scan results cleanup: {e}")


# âœ… Start cleanup thread when server starts
...
def get_dealer_customers():
    ...
    """
    try:
        # âœ… SECURITY FIX: Validate and convert dealer_id
        ...
        # âœ… SECURITY FIX: Verify user is actually a dealer
        dealer_code = get_dealer_code(dealer_id)
        if not dealer_code:
            return jsonify({"message": "Access denied: user is not a dealer"}), 403
        
        # âœ… IMPROVEMENT: Validate pagination parameters safely
        try:
            limit = int(request.args.get("limit", 50))
            if limit <= 0:
                limit = 50
        ...
        # âœ… IMPROVEMENT: Fetch data with error handling
        rows = get_customers_by_dealer_code(dealer_code, limit=limit, offset=offset)
        
        # âœ… IMPROVEMENT: Build response with safer data extraction
        customers = []
        for row in rows:
...
#         print("=================================================")
#         print(f"ðŸ“¦ Devices count: {len(clean_devices)}")
#         print("ðŸ“¡ Forwarding payload to remote URLs (in order):")
#         for u in remote_urls:
#             print("   ->", u)
#         print("=================================================")

#         # âœ… IMPORTANT: Use Session + disable env proxy
#         session = requests.Session()
#         session.trust_env = False  # âœ… ignores HTTP_PROXY / HTTPS_PROXY from environment

#         last_attempt_details = []
#         for remote_url in remote_urls:
#             try:
#                 print(f"\nâž¡ï¸ TRY remote_url = {remote_url}")
...
#                     allow_redirects=False,     # âœ… so we can see redirect target
#                     proxies={"http": None, "https": None}  # âœ… force no proxy
#                 )

#                 # âœ… capture redirect info (if any)
#                 location = resp.headers.get("Location")
#                 server_hdr = resp.headers.get("Server")
#                 via_hdr = resp.headers.get("Via")
#                 xff = resp.headers.get("X-Forwarded-For")

#                 print("âœ… status_code =", resp.status_code)
#                 print("âœ… resp.url    =", resp.url)
#                 print("âœ… Location    =", location)
#                 print("âœ… Server      =", server_hdr)
#                 print("âœ… Via         =", via_hdr)
#                 print("âœ… XFF         =", xff)
#                 print("âœ… body(head)  =", (resp.text or "")[:300])
...
#                 # âœ… If redirect happened, stop and show it clearly
...
#                 # âœ… FIXED: Check response status correctly
...
#                 print(f"âœ… Remote status: {remote_status}, HTTP status: {resp.status_code}, OK: {ok}")

#                 if ok:
#                     # âœ… Forward the remote response back to client
#                     return jsonify({
#                         "ok": True,
...
#                 print(f"âš  Remote returned not-ok status: {remote_status}. Trying next URL (if any)...")
...
#                     "details": str(e),
#                 })
#                 print("ðŸ”Œ ConnectionError on", remote_url, str(e))
...
#                     "details": str(e),
#                 })
#                 print("âŒ Exception on", remote_url, str(e))
...
def api_scan_db():
    ...
    try:
                ...
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    if result:
                        ...
                        if serial_row:
                            ...
                            print(f"   ðŸ“Š TARGET_IP: {TARGET_IP}, agent_id: {agent_id}")
        except Exception as e:
            print(f"âŒ Database error: {e}")
            return jsonify({
                "ok": False,
        ...
        # âœ… IMPORTANT: Send COMPLETE scan data as received
        ...
        print("=" * 80)
        print(f"ðŸ“¦ Total devices: {len(devices)}")
        print(f"ðŸŽ¯ Target agent_id: {agent_id}")
        print(f"ðŸ“¡ Sending complete scan data via socket...")
        print(f"ðŸ“„ Sample device: {json.dumps(devices[0] if devices else {}, indent=2)}")
        print("=" * 80)

        # âœ… Send complete scan data via socket
        try:
            print(f"\nðŸ”Œ Sending 'receive_scan_data' command to agent {agent_id}...")
            ...
            print(f"âœ… Socket response received:")
            ...
            if socket_response.get("ok") or socket_response.get("status") == "ok":
                ...
                print(f"âŒ Socket command failed: {error_msg}")
            ...
            print(f"âŒ Socket error: {str(e)}")
            traceback.print_exc()
        ...
        print(f"âŒ Fatal error in api_scan_db:")
        traceback.print_exc()
        return jsonify({
            "ok": False,
            "message": "Internal server error",
...
# 
# Dashboard â†’ these server routes â†’ socket_hub_send_command â†’ agent (clienttt.py)
...
def _get_agent_ip_for_proxy():
    ...
    if not TARGET_IP:
        print(f"âŒ [proxy] ip_data={ip_data!r} â€” no valid non-loopback IP found")
        raise Exception("No valid (non-loopback) agent IP found")

    print(f"ðŸŽ¯ Proxy â†’ agent IP: {TARGET_IP}")
...
def _proxy_via_socket(agent_id, command, data=None, timeout=30):
    ...
    print(f"ðŸ”Œ Socket response for [{command}]: {resp}")
    return resp


# â”€â”€â”€ ANALYTICS: GET all â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def proxy_get_analytics():
    ...
    try:
        ...
        if resp.get("ok") or resp.get("status") == "ok":
            ...
            print(f"âš ï¸  [analytics] Agent returned error: {err_msg}")
...
# â”€â”€â”€ ANALYTICS: GET by id â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def proxy_get_analytic_by_id(analytic_id):
    ...
    try:
        ...
        if resp.get("ok") or resp.get("status") == "ok":
            ...
            err_msg = resp.get("error") or resp.get("message", "Agent error")
            print(f"âš ï¸  [analytics/{analytic_id}] Agent returned error: {err_msg}")
            return jsonify({"status": "error", "message": err_msg}), 503
...
# â”€â”€â”€ ANALYTICS: PUT (update) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def proxy_update_analytic(analytic_id):
    ...
    try:
        ...
        if resp.get("ok") or resp.get("status") == "ok":
            ...
            err_msg = resp.get("error") or resp.get("message", "Agent error")
            print(f"âš ï¸  [analytics/{analytic_id} PUT] Agent returned error: {err_msg}")
            return jsonify({"status": "error", "message": err_msg}), 503
...
# â”€â”€â”€ ANALYTICS: DELETE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def proxy_delete_analytic(analytic_id):
    ...
    try:
        ...
        if resp.get("ok") or resp.get("status") == "ok":
            ...
            err_msg = resp.get("error") or resp.get("message", "Agent error")
            print(f"âš ï¸  [analytics/{analytic_id} DELETE] Agent returned error: {err_msg}")
            return jsonify({"status": "error", "message": err_msg}), 503
...
# â”€â”€â”€ DEVICES: GET all â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def proxy_get_devices():
    ...
    try:
        ...
        if resp.get("ok") or resp.get("status") == "ok":
            ...
            err_msg = resp.get("error") or resp.get("message", "Agent error")
            print(f"âš ï¸  [devices] Agent returned error: {err_msg}")
            return jsonify({"status": "error", "message": err_msg}), 503
...
# â”€â”€â”€ DEVICES: PUT (update) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def proxy_update_device(device_id):
    ...
    try:
        ...
        if resp.get("ok") or resp.get("status") == "ok":
            ...
            err_msg = resp.get("error") or resp.get("message", "Agent error")
            print(f"âš ï¸  [devices/{device_id} PUT] Agent returned error: {err_msg}")
            return jsonify({"status": "error", "message": err_msg}), 503
...
# â”€â”€â”€ DEVICES: DELETE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def proxy_delete_device(device_id):
    ...
    try:
        ...
        if resp.get("ok") or resp.get("status") == "ok":
            ...
            err_msg = resp.get("error") or resp.get("message", "Agent error")
            print(f"âš ï¸  [devices/{device_id} DELETE] Agent returned error: {err_msg}")
            return jsonify({"status": "error", "message": err_msg}), 503
